...
//        curl_setopt($curl, CURLOPT_SSL_CIPHER_LIST, 'SSLv3');
        ...
        if (curl_errno($curl)) {
            ...
            $this->_params = $params;
            curl_close($curl);
            $logger->log("CURL error n° " . print_r($this->_errors, true) . ' - request: '.print_r($orig_params).' - response: '. print_r($response, true), Zend_Log::DEBUG);
            ...
            if ($responseArray['ACK'] === 'Success') {
                ...
                $this->_params = $params;
                curl_close($curl);
                $logger->log("CURL error: ".print_r($this->_errors, true). ' - request: '.print_r($orig_params, true), Zend_Log::DEBUG);
                return false;
            }
        }
    }
