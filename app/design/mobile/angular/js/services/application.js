App.service("Application", function($rootScope, $window, $http, $q, $timeout, $interval, $location, Push, modalManager, Url, Translator) {
    ...
    service.setDeviceUid = function(device_uid) {
        ...
        if(service.is_native) {

            Push.getPushs(device_uid);

            Push.getLastMessages(device_uid).success(function(data) {
                if(data) {
                    //Loading last push
                    ...
                    if (data.inapp_message) {
                        var inapp_modal = {
                            ...
                            "confirmAction": function() {Push.markInAppAsRead(device_uid, service.is_ios ? 1 : 2);}
                        };
